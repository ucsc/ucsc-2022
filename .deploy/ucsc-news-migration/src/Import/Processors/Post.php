...
class Post {
	...
	protected function inject_co_authors( array $author, int $post_id ) {
		...
		$query 			 = $wpdb->prepare( "SELECT ID FROM {$wpdb->posts} WHERE post_type = '%s' AND post_status = 'publish' AND post_name = '%s'", 'guest-author', sprintf( 'cap-%s', sanitize_title( $author['name'] ) ) );
		$existing_author = $wpdb->get_var( $query );

		// Create new author
		if ( ! $existing_author ) {
			$existing_author = [
				'post_name'   => sprintf( 'cap-%s', sanitize_title( $author['name'] ) ),
			...
			$meta_fields = [
				'cap-display_name' => $author['name'],
				'cap-first_name'   => '',
				'cap-last_name'    => '',
				'cap-user_login'   => sanitize_title( $author['name'] ),
				'cap-user_email'   => $author['email'] ?: '',
				'cap-website'      => '',
				'cap-description'  => '',
			];
		...
		$co_authors_plus->add_coauthors( $post_id, [ $author['name'] ], true );
	}
