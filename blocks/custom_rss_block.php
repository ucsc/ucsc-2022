 ...
 */
function custom_rss_block_template( $attributes ) {
	...
	foreach ( $rss_items as $item ) {
		$item->get_thumbnail();
		if ($attributes['rssimage']) {
			// Display an image with this value saved and set to true
		}
		...
		if ( $attributes['displayExcerpt'] ) {
			...
			$excerpt = '<div class="wp-block-rss__item-excerpt">' . esc_html( $excerpt . ' ---- MY CUSTOM CONTENT HERE!!!!' ) . '</div>';
		}
 ...
 */
function register_block_custom_rss() {
	unregister_block_type( 'core/rss' );

	register_block_type(
		ABSPATH . '/wp-includes/blocks/rss',
		array(
			'render_callback' => 'custom_rss_block_template',
		)
	);
}

add_action( 'init', 'register_block_custom_rss' );
